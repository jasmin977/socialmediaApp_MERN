...
const msg = {
    "en-US": " sent you a heart request.",
    "fr-FR": " vous a envoyé un coup de coeur.",
}


const NotifHeartRequest = (props) => {
    const { notif } = props;
    ...
    const currUser = users.filter(u => u._id === notif.heartrequester)[0];
    ...
    const vuNotif = async (notifId) => {
        try {
            await dispatch(notifAcctions.heartReqNotifSeen(notifId));
        } catch (err) {
            console.log(err);
        }
    ...
    return (
        <TouchableHighlight
            ...
            <View style={notif.VuHeartReq ? styles.unseennotif : styles.notif}>
                ...
                <View style={{
                    ...
                    <ImageBackground
                        source={{ uri: `${ENV.apiUrl}/user/photo/${notif.heartrequester}` }}
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <Image
                                ...
                                source={notif.VuHeartReq ? require('../../assets/pic_cover.png') : require('../../assets/pic_cover_stars.png')}
...
export default NotifHeartRequest;