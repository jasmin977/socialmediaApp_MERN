...
const msg = {
    "en-US": " accepted your heart request",
    "fr-FR": " a accepté votre coup de coeur",
}




const NotifHeartAccept = (props) => {
    const { notif } = props;
    ...
    const currUser = users.filter(u => u._id === notif.heartaccepter)[0];
    ...
    const vuNotif = async (notifId) => {
        try {
            await dispatch(notifAcctions.heartAcceptNotifSeen(notifId));
        } catch (err) {
            console.log(err);
        }
    ...
    return (
        <TouchableHighlight
            ...
            <View style={notif.VuHeartAcc ? styles.unseennotif : styles.notif}>
                ...
                <View style={{
                    ...
                    <ImageBackground
                        source={{ uri: `${ENV.apiUrl}/user/photo/${notif.heartaccepter}` }}
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <Image
                                ...
                                source={notif.VuHeartAcc ? require('../../assets/pic_cover.png') : require('../../assets/pic_cover_stars.png')}
...
export default NotifHeartAccept;