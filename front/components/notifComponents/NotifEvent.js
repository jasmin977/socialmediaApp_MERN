...
import { View, Text, StyleSheet, Image, ImageBackground, Animated, TouchableHighlight, ActivityIndicator, Platform, Alert, TouchableNativeFeedback, Dimensions } from 'react-native';
...
const msg = {
    "en-US": " participated in your event",
    "fr-FR": " a participé dans votre evenement",
}




const NotifEvent = (props) => {
    const { notif } = props;
    ...
    const currUser = users.filter(u => u._id === notif.participant)[0];
    ...
    const vuNotif = async (notifId) => {
        try {
            await dispatch(notifAcctions.participateNotifSeen(notifId));
        } catch (err) {
            console.log(err);
        }
    ...
    return (
        <TouchableHighlight
            ...
            <View style={notif.VuEventPart ? styles.unseennotif : styles.notif}>
                ...
                <View style={{
                    ...
                    <ImageBackground
                        source={{ uri: `${ENV.apiUrl}/user/photo/${notif.participant}` }}
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <Image
                                ...
                                source={notif.VuEventPart ? require('../../assets/pic_cover.png') : require('../../assets/pic_cover_stars.png')}
