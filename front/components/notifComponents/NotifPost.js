...
const msg = {
    "en-US": " liked your post",
    "fr-FR": " aimé votre publication",
}



const NotifPost = (props) => {
    const { notif } = props;
    ...
    const currUser = users.filter(u => u._id === notif.liker)[0];
    ...
    const vuNotif = async (notifId) => {
        try {
            await dispatch(notifAcctions.likeNotifSeen(notifId));
        } catch (err) {
            console.log(err);
        }
    ...
    return (
        <TouchableHighlight
            ...
            <View style={notif.VuPostLike ? styles.unseennotif : styles.notif}>
                ...
                <View style={{
                    ...
                    <ImageBackground
                        source={{ uri: `${ENV.apiUrl}/user/photo/${notif.liker}` }}
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <Image
                                ...
                                source={notif.VuPostLike ? require('../../assets/pic_cover.png') : require('../../assets/pic_cover_stars.png')}
